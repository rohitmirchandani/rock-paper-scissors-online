            ...
            if(message.event=='userwanttojoin' && message.username!==''){
                ...
                for(let i=0;i<USERS.length;i++){
                    if(message.username === USERS[i].username){
                        let requestmsg = {'event':'userwanttoplay','username':currentuser.username}
                        USERS[i].client.send(JSON.stringify(requestmsg))
                        break;
                    }
                }
            }else if(message.event == 'yesiwanttoplay' || message.event=='noidontwanttoplay'){
                ...
                let replymsg = {'event':'move','username':currentuser.username,'move':message.move}
                search(message.username).client.send(JSON.stringify(replymsg))
    ...
    socketclient.on('close',()=>{
        ...
        try{
            ...
            if(currentuser!=undefined && currentuser.username !== undefined){
                let broadcastmsg =  {'event':'userleft','username':currentuser.username}
                USERS.forEach((user)=>{
                    user.client.send(JSON.stringify(broadcastmsg))
                })
            }
